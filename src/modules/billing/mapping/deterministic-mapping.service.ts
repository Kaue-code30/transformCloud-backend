...
const TARGET_PROVIDERS: CloudProvider[] = ['AWS', 'GCP', 'AZURE', 'OCI'];

interface SourceResourceSpecs {
  offeringId?: string;
...
export class DeterministicMappingService {
    ...
    for (const service of billing.topServices) {
      ...
      if (!source) {
        ...
        unmapped.push({
          ...
          reason:
            'Serviço sem estratégia determinística compatível ou SKU/capacidade não encontrada no catálogo',
        });
        continue;
      }

      const mapping: ServiceMapping = { original: service.name };
      const failures: string[] = [];
      for (const targetProvider of TARGET_PROVIDERS.filter(
        (provider) => provider !== billing.provider,
      )) {
        const targetRegion = resolveTargetRegion(
          billing.targetRegion ?? source.region,
          targetProvider,
        );
        if (!targetRegion) {
  ...
  private async resolveSource(
  ...
  ): Promise<SourceResourceSpecs | null> {
    const nativeSkuName =
      service.nativeSkuName ??
    ...
    const offering = await this.findSourceOffering(
      provider,
      nativeSkuName,
      nativeProductId,
      region,
    ...
    if (offering) {
      if (
        offering.resourceKind === 'COMPUTE_VM' &&
    ...
    // Fallback sem oferta de origem só é seguro para VMs com capacidade explícita.
    if (!isComputeService(provider, service, lineItem)) return null;

    const vcpu = service.vcpu ?? numericAttribute(lineItem, ['vcpu', 'vcpus', 'cores']);
    const memoryGiB =
      service.memoryGiB ?? numericAttribute(lineItem, ['memoryGiB', 'memory_gib', 'memory']);
    if (!nativeSkuName || !region || !vcpu || !memoryGiB) return null;
  ...
  private async findSourceOffering(
    provider: CloudProvider,
    nativeSkuName?: string,
    ...
    if (!nativeSkuName && !nativeProductId) return null;
  ...
  private async findTarget(
    source: SourceResourceSpecs,
    targetProvider: CloudProvider,
    targetRegion: string,
  ): Promise<{ offering: CatalogOfferingView; metadata: MappingMetadata } | null> {
    if (source.offeringId) {
      const override = await this.catalog.findApprovedOverride(
        source.offeringId,
        targetProvider,
      );
    ...
    const candidates = source.resourceKind === 'COMPUTE_VM'
      ? await this.catalog.findComputeCandidates({
          provider: targetProvider as CatalogProvider,
          region: targetRegion,
    ...
    if (
      source.resourceKind !== 'COMPUTE_VM' &&
      source.vcpu == null &&
    ...
    const matchType: MappingMatchType = exactCapacity
      ? 'exact_capacity'
...
function assignProviderMapping(
  mapping: ServiceMapping,
  offering: CatalogOfferingView,
  ...
  if (offering.provider === 'AWS') {
    ...
    mapping.azure = {
      ...
      region: offering.region,
      confidence,
    } satisfies AzureMapping;
  } else {
    mapping.oci = {
      ...metadata,
      service: offering.serviceName,
      shape: offering.nativeSkuName,
      // OCPU e vCPU não são equivalentes em todas as arquiteturas OCI.
      // O valor só deve ser preenchido por um normalizador/override específico.
      memoryGb: offering.memoryGiB ?? undefined,
      confidence,
    } satisfies OciMapping;
  }
}

function scoreResource(source: SourceResourceSpecs, target: CatalogOfferingView): number {
...
function compatiblePricingUnits(unit: string | null): string[] | undefined {
  const normalized = (unit ?? '').toLowerCase().replace(/[\s_-]/g, '');
  if (/^(hrs?|hours?|horas?)$/.test(normalized)) return ['Hrs', 'hour', 'Hour', 'h'];
  if (['gbsecond', 'gibsecond', 'gibys', 'gbs'].includes(normalized)) return ['GB-Second', 'GB-s', 'GiB-s', 'GiBy.s'];
  if (['gbmo', 'gbmonth', 'gibymo'].includes(normalized)) return ['GB-Mo', 'GB-month', 'GiBy.mo'];
  if (['gb', 'gib', 'giby'].includes(normalized)) return ['GB', 'GiB', 'GiBy'];
  return unit ? [unit] : undefined;
}

function buildEvidence(
  source: SourceResourceSpecs,
  target: CatalogOfferingView,
...
function confidenceFromScore(score: number, matchType: MappingMatchType): Confidence {
  if (matchType === 'manual_override' || score <= 0.05) return 'high';
...
function findLineItem(
  ...
  return lineItems.find((item) => {
    ...
    return candidates.some(
      (candidate) => candidate.includes(serviceName) || serviceName.includes(candidate),
    );
  });
}

function numericAttribute(
  item: BillingLineItem | undefined,
  keys: string[],
): number | undefined {
...
function extractSkuFromSpecs(provider: CloudProvider, specs: string): string | undefined {
  if (provider === 'AZURE') return specs.match(/Standard_[A-Za-z0-9_]+/i)?.[0];
  if (provider === 'AWS') return specs.match(/(?:db\.)?[a-z][a-z0-9]*\.[a-z0-9]+/i)?.[0];
  if (provider === 'GCP') return specs.match(/[a-z][a-z0-9]*-(?:standard|highmem|highcpu|micro|small|medium)-\d+/i)?.[0];
  return specs.match(/VM\.[A-Za-z0-9.]+(?:\.Flex)?/i)?.[0];
}

function extractRegionFromSpecs(provider: CloudProvider, specs: string): string | undefined {
  if (provider === 'AWS') return specs.match(/[a-z]{2}(?:-gov)?-[a-z]+-\d/i)?.[0];
  if (provider === 'GCP') return specs.match(/[a-z]+-[a-z]+\d/i)?.[0];
  const lower = specs.toLowerCase();
  return ALL_REGION_ALIASES.find((region) => lower.includes(region));
}

function normalizeOperatingSystem(value: string | undefined): string | null {
  const lower = value?.toLowerCase() ?? '';
  if (lower.includes('windows')) return 'Windows';
  if (lower.includes('linux') || lower.includes('ubuntu') || lower.includes('rhel')) return 'Linux';
  return null;
}

function normalizeArchitecture(value: string | undefined): string | null {
  const lower = value?.toLowerCase() ?? '';
  if (lower.includes('arm64') || lower.includes('aarch64') || lower.includes('graviton')) return 'arm64';
...
function isComputeService(
  ...
  const markers: Record<CloudProvider, string[]> = {
    AWS: ['amazonec2'],
  ...
  return identities.some((identity) =>
    markers[provider].some((marker) => identity.includes(marker)),
  );
}
...
function resolveTargetRegion(
  ...
  const withoutZone = normalized.replace(
    /([a-z]{2}(?:-gov)?-[a-z]+-\d)[a-z]$/,
    '$1',
  );
  const group = REGION_GROUPS.find(({ aliases, AWS, GCP, AZURE, OCI }) =>
    [...aliases, AWS, GCP, AZURE, OCI].some((value) => value.toLowerCase() === withoutZone),
  );
